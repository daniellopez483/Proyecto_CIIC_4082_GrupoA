...
player_jump: .res 1   ;a byte to count the height of the player on a jump (starts at 0 and if Jump is called, atarts incrementing until it reaches $31)
pointerLo: .res 1 ;One-byte low memory address variable
pointerHi: .res 1 ;One-byte high memory address variable
.exportzp player_x, player_y, player_dir, pad1, player_jump    ;export their definition from reset.asm
...
check_down:
  ...
  BEQ check_A
  ;check lower bound
  ;;;;;;;;;;;;;;;;;;;;;comparacion en la plataforma
  ;check if mario is in the platform
  LDA player_x
  CMP #$20
  BCC not_on_platform
  LDA player_x
...
not_on_platform:
  LDA player_y
  CMP #$bf            ;e0 minus 09 - $20 +8so lower half of the sprite does not get hidden
  BCS check_A ;;;;;;;;;;;;;change to check A
  INC player_y
check_A:
  LDA pad1
  AND #BTN_A
...
up_left:
  ...
  LDA pad1
  AND #BTN_DOWN
  BEQ no_gravity      ;if gravity is off
  LDA pad1
  EOR #%00000100
  STA pad1
no_gravity:
...
check_jump:
  LDA player_jump
  CMP #$00
  BEQ stop_rising   ;if we are not jumping
  DEC player_y          
  INC player_jump
  LDA player_jump  
  CMP #$41
  BCC stop_rising     ;if we have jumped for $41 frames
  JMP done_checking
  ;restore "gravity"
stop_rising:
  LDA #$00
...
facing_left:
  ...
  ;if facing up
facing_up_right:
  LDA #$1a    ;tile number (top left part)
  STA $0201   ;memory address (they correspont to "byte 2" of the first four sprites)
  LDA #$1b    ;tile number (top right)
  STA $0205
  LDA #$1c    ;tile (bottom left)
  STA $0209
  LDA #$1d    ;tile (bottom right)
  STA $020d
